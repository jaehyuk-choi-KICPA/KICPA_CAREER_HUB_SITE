...
_DEFAULTS: dict = {
    ...
    "dashboard": {
        ...
        # 딜·M&A 한정 제외 — '매각'이 끌어오는 부동산·압류재산성 노이즈(아파트/주택/토지/공매 등)는 M&A 아님.
        # 카테고리 스코프라 세무의 '부동산 매각 양도세' 같은 정상기사는 무사.
        "news_deal_exclude": ["아파트 매각", "부동산 매각", "주택 매각", "토지 매각", "상가 매각",
                              "오피스텔", "빌라 매각", "압류재산", "분양권",
                              "체비지", "보류지",   # 재개발 사업비 토지(체비지·보류지) 매각 공고
                              "대가성",   # 정치성 매각 스캔들(예: '집 헐값 매각, 대가성 의심') — 딜에선 항상 정치·뇌물건
                              "사옥 매각", "빌딩 매각", "타워 매각", "용적률",   # 오피스·건물 부동산 딜(예: 'GS건설 서초타워 매각…용적률 800%')
                              "공공자산",   # 지자체 공공자산 매각 정치·행정 기사(예: '북구 공공자산, 매각보다 공공성')
                              "목표가", "목표주가", "투자의견",   # 증권사 애널리스트 리포트(밸류에이션 쿼리 오매칭) — 딜 아님
                              "ACQUISITION CORP", "Acquisition Corp"],   # 미국 SPAC 쉘 공시 번역물(나스닥 상폐·거래정지 등)
        ...
        "industry_foreign_filter": True,    # news_foreign_* 어휘를 공유해 외국 단독 기사 차단
        ...
        "industry_companies": {
            ...
            "에쓰오일":     {"industries": ["에너지·화학"], "aliases": ["S-Oil", "에스오일"]},
            "LG화학":       {"industries": ["에너지·화학"], "aliases": []},
            "롯데케미칼":   {"industries": ["에너지·화학"], "aliases": []},
            "한국전력":     {"industries": ["에너지·화학"], "aliases": ["한전"]},
            "포스코홀딩스": {"industries": ["철강·소재"], "aliases": ["포스코"]},
            ...
            "SK바이오팜":   {"industries": ["제약·바이오"], "aliases": []},
            ...
            "삼성화재":     {"industries": ["금융·보험"], "aliases": []},
            "네이버":       {"industries": ["IT·플랫폼·게임"], "aliases": ["NAVER"]},
            ...
            "KT":           {"industries": ["통신·미디어·엔터"], "aliases": [], "strict": True},
