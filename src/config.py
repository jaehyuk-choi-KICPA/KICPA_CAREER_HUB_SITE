...
_DEFAULTS: dict = {
    ...
    "dashboard": {
        ...
        "news_embed_threshold": 0.85,            # 코사인 이 값↑이면 같은 사건으로 병합. 0.82=과병합(선발 과도↔수습처 확대
                                                 # 등 별개사건이 전이연결로 합쳐짐), 0.88=과소병합(발전공기업 통합 동일사건이 4카드로 흩어짐).
                                                 # 0.85=중간값(실측: 발전통합 4→2카드 병합·금감원 덤핑 18건 깔끔·선발↔수습처는 여전히 분리).
