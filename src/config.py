...
_DEFAULTS: dict = {
    ...
    "dashboard": {
        ...
        # 카테고리별 보존기간 override — 저빈도·고관련(채용/시험·업계동향)은 더 오래 노출
        ...
        # 기사=면접·업계지식 중심. 좁은(타깃 직결) 카테고리를 먼저 둬 중복제거 시 선점하게 함
        # (build_news가 이 순서대로 dedup → 뒤의 넓은 쿼리에 겹치는 기사를 빼앗기지 않음).
        "news_queries": {
            "채용·시험": "수습 공인회계사 OR 공인회계사 시험 OR 한국공인회계사회 OR CPA 합격 OR 회계사 채용 OR 회계법인 채용",
            "딜·M&A": ("인수합병 OR M&A OR IPO OR 기업가치평가 OR 구조조정 OR 사모펀드 회계법인 OR "
                       "실사 회계법인 OR 재무자문 회계법인"),
            "회계업계": ("회계법인 OR 빅4 회계 OR 공인회계사 업계 OR 삼일 삼정 안진 한영 회계 OR "
                         "감사보수 OR 회계사 연봉 OR 회계법인 실적"),
