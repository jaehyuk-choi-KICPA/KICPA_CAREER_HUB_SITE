...
- **진단(단계별 funnel 계측):** 두 가지 병목이 겹침.
  ...
  2. **임베딩 의미군집 임계(`news_embed_threshold`)가 너무 낮음(0.82).** 짧은 한국어 제목에서 임베딩이
     ...
     → 먼저 0.88로 올렸더니 이번엔 **과소병합**(발전공기업 통합 동일사건이 4카드로 흩어짐) → 0.82와 0.88의
     **중간값 0.85**로 정착(실측: 발전통합 4→2카드·금감원 덤핑 18매체 깔끔 동일사건·선발↔수습처는 여전히 분리).
...
- **무엇을/어디에:** `config.py` `dashboard` — `news_per_category` 50→75, `news_embed_threshold` 0.82→**0.85**,
  `news_recent_days_by_category['딜·M&A']` 35→60. (임베딩은 끄지 않고 임계만 조정 — 동일사건 군집은 유지.)
- **효과/검증:** 키 ON 재빌드 시 딜 0건 사고 해소(딜 12건 안정), 동일사건(발전공기업 통합·금감원 덤핑 등) 묶이고
  별개사건(선발 과도↔수습처 확대)은 분리. **임계 튜닝 교훈: 0.82=과병합 / 0.88=과소병합 / 0.85=균형.**
