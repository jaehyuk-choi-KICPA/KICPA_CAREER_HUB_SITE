...
- **기사 수량 레버**(58→116 실측, 종합 7.9→8.9): 안전=`news_per_category`(20→50→**75**)→풀린 뒤엔 **`news_recent_days`(→21)**가
  ...
  - ⚠️ **`news_embed_threshold`는 양날**: 0.82↓=과병합(별개사건이 union-find 전이연결로 한 카드 — 선발 과도 발언↔수습처
    확대 발표), 0.88=과소병합(발전공기업 통합 동일사건이 4카드로 흩어짐). **0.83=동일사건 적극병합**(발전통합 1카드,
    선발↔수습처는 여전히 분리). 표시 대표 수가 비정상적으로 적으면(딜 0건 등) per_category·threshold·딜 recency(60)부터 의심.
    카테고리별 건수는 구글뉴스 RSS draw 변동으로 매 수집 출렁임(채용이 1건까지도 — 임계 탓 아님).
