...
1. **채용공고** — KICPA 2보드 + 삼정·안진·한영·삼일(6어댑터). 분류: 법인(삼일/삼정/안진/한영/로컬/기타) ×
   **자격요건(수습CPA/자격무관)** × **채용구분(인턴/정규직/계약직/파트타임)** × 상태(진행중/마감) × NEW(게시 24h).
   ...
   ① **빅4 신입 공채**(삼일·삼정·안진·한영 정기채용 일정, 접이식·기본 닫힘): **수동 큐레이션 `docs/data/big4_recruit.json`**
   (export 파이프라인과 분리, 시즌마다 수기 갱신). 법인색 좌측 바. ② **로컬 신규공채**(접이식·기본 열림): jobs.json에서
   로컬×수습CPA×미마감(제목 '(마감)' 가드 포함)을 자동 필터해 **풀타임 ↔ 파트타임 하위 탭**으로 표시(마감 임박순, 5개+더보기 — `renderLocalRecruit`).
