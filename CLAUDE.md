...
2. **기사(업계)** — Google News RSS(`adapters/news_rss.py`), 4분류(채용·시험/감사/세무/딜·M&A). 제목+출처+링크만, 노이즈 제외, 기본 21일 보존(카테고리별 차등).
...
3. **빅펌 인사이트** — 삼일·삼정·안진·한영 간행물(`insights.py`). 사이트가 JS(SPA)라 **Playwright 헤드리스**(`render.py`)로 렌더 후 링크 추출.
   (프론트는 **한 탭('기사·산업') 안 책갈피 3개** — 📰업계 / 🏭산업 / 📑리포트. 백엔드 데이터는 그대로 분리.)
3-1. **기업 정보(DART)** — `src/adapters/dart.py`가 **공식 Open API**로 기업별 **감사인·3개년 주요계정·최근 공시**를
   모아 `docs/data/companies.json` 생성(주 1회). 감사보고서 공시의 `flr_nm`(제출인)이 곧 그 회사의 감사인이다.
