...
## 2026-06-21 (6) — '신입불가' 예외 오구제 버그 수정(경력공고 누출)

- **증상/계기:** 일반 CPA 보드(`kicpa_cpa`)의 **'세무기장 경력 3년이상 모집(신입불가)'**(서율회계법인)가 NEW로 노출. 제목에 '3년이상'(hard 제외 대상)이 있는데 **'신입불가'의 '신입'이 예외어로 부분일치**→`경력+신입 병기`로 오인해 오구제→통과. (사용자는 스크랩 범위가 넓어진 것으로 오인했으나, `kicpa_cpa`는 init부터 수집한 KICPA 2보드 중 하나로 **범위 변경 없음** — 순수 필터 버그.)
- **무엇을 / 어디에:** `config.filters`에 **`exception_negators`**(불가·제외·아님 등) 신설. `filters._exception_present(s, exceptions, negators)` 추가 — 예외어 바로 뒤에 부정어가 붙으면 예외로 치지 않음. `passes`의 두 예외 검사(제목 hard·본문 soft)를 `any(exc in ...)` → `_exception_present(...)`로 교체.
- **효과/검증:** 단위검증 — 서율 제목→제외, '경력/신입' 진짜 병기→유지, 수습보드(kicpa_susup) 동일 제목은 bypass로 유지. 회귀 테스트 추가(`test_negated_exception_not_rescued`), 전체 **67 통과**. 재수집 시 서율 공고 제외 확인(kicpa_cpa 10→8).
