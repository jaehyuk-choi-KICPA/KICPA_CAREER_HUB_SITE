...
- **진단(단계별 funnel 계측):** 두 가지 병목이 겹침.
  ...
  2. **임베딩 의미군집 임계(`news_embed_threshold`)가 너무 낮음(0.82).** 짧은 한국어 제목에서 임베딩이
     ...
     → **0.88로 상향**(실측: 발언↔발표 분리·동일사건(권혁 세무조사 15매체 등)은 유지, 최종 40→89).
...
- **무엇을/어디에:** `config.py` `dashboard` — `news_per_category` 50→75, `news_embed_threshold` 0.82→0.88,
  `news_recent_days_by_category['딜·M&A']` 35→60. (임베딩은 끄지 않고 임계만 보수화 — 동일사건 군집은 유지.)
- **효과/검증:** 키 ON 재빌드 시 총 **85건**(세무 34·감사 22·채용 15·**딜 14**)으로 안정 복원, 딜 0건 사고 해소.
  최대 클러스터는 권혁 세무조사 15매체(정상 동일사건). 선발 과도 카드의 수습처 오흡수 사라짐.
