...
- **dupes 평면화 + gid**: 군집을 중첩 저장하면 대표의 dupes 배열이 스냅샷마다 합쳐지며 무한 누적된다
  (실측 11MB). 대표·멤버를 각각 독립 레코드로 펴서 3MB로 줄였다. 다만 관계까지 버리면 라이브에서
  카드 1장이던 사건이 아카이브에선 5장으로 흩어지므로, **대표 URL을 `gid`로 스탬프**해 관계만 남긴다
  (문자열 1개라 누적되지 않는다). 프론트 `foldByGid()`가 읽을 때 다시 접어 '동일 주제 기사 N개'로 보여준다.
- **제목 가드**: 구글뉴스 RSS는 같은 기사를 시점에 따라 **다른 base64 링크**로 내보낸다. URL만 키로
  삼으면 그때마다 새 레코드가 되므로(실측 업계 19%), `merge_items`가 정규화 제목이 같으면 기존
  레코드를 보강한다. 과거분은 `scripts/archive_dedup.py`로 일회 정리했다.
