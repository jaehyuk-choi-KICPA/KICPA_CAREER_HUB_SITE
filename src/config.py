...
_DEFAULTS: dict = {
    ...
    "dashboard": {
        ...
        "news_embed_threshold": 0.88,            # 코사인 이 값↑이면 같은 사건으로 병합. 0.82는 과병합(별개사건이
                                                 # union-find 전이연결로 묶임 — 예: 한공회장 '선발 과도' 발언이 금융위 '수습처 확대' 발표와 합쳐짐).
                                                 # 0.88로 상향(실측: 발언↔발표 분리·동일사건(권혁 세무조사 15건 등)은 유지, 최종 53→97로 회복).
