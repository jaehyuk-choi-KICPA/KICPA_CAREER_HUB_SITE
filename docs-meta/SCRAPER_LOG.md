...
## 2026-08-28 (30) — 아카이브 중복 정리: 제목 재유입 차단 + 군집 id(gid) 보존

- **증상/계기:** '전체 기간' 화면에 같은 기사가 여러 장 떴다. 사용자가 "Voyage가 업계 기사에도
  제대로 적용되는지 의문"이라고 지적. 확인 결과 **임베딩은 정상 작동**(news_vectors.json 매 수집 갱신,
  라이브 스냅샷 39카드에 75건이 dupes로 흡수, 잔여 근접중복 0건)이었고 문제는 아카이브 계층에 있었다.
- **원인 두 가지:**
  1. **같은 제목·다른 링크** — 구글뉴스 RSS는 같은 기사를 시점에 따라 다른 base64 링크로 내보낸다.
     `merge_items`가 URL만 키로 삼아 그때마다 새 레코드가 됐다. 실측 업계 2,888건 중 **543건(19%)**,
     산업 4,259건 중 146건. (`norm_url`은 쿼리만 떼므로 경로 토큰 변화를 못 잡는다.)
  2. **평면화된 군집** — `_expand`가 라이브의 `대표 + dupes`를 독립 레코드로 편다(배열을 그대로 담으면
     스냅샷마다 누적돼 11MB까지 부푼 실측 사고 때문). 그 결과 라이브에서 카드 1장이던 사건이
     아카이브에선 5장으로 흩어졌다. **저장 구조는 옳았고, 잃어버린 건 군집 '관계'였다.**
- **무엇을 / 어디에:**
  - `archive.norm_title()` + `merge_items`의 **제목 가드** — URL이 달라도 정규화 제목이 같으면
    새 레코드 대신 기존 레코드를 보강한다(최초 레코드·first_archived 유지).
  - `_expand`가 대표 URL을 **`gid`로 스탬프** — 배열이 아닌 문자열 한 개라 누적되지 않는다.
    `_KEEP`에 `gid` 추가, `_merge_one`이 빈 gid를 채운다.
  - `docs/app.js` `foldByGid()` — `archAll`이 읽은 뒤 같은 gid를 다시 접어 라이브와 동일한
    '동일 주제 기사 N개' 카드로 만든다(정보 보존 + 화면 정돈).
  - `scripts/archive_dedup.py` — 과거분 일회성 정리. 제목 중복 제거 + 라이브와 **같은 판정**
    (`_same_issue`)으로 gid 소급 부여. `--dry-run` 지원.
- **효과/검증:**
  - 업계 아카이브 2,888건 → **2,345 레코드**(제목중복 543 제거) → 화면 **1,845 카드**(군집 500 접힘).
  - 산업 4,259건 → 4,113 레코드 → 화면 **3,741 카드**. 아카이브 용량 6.0MB → 4.6MB.
  - Playwright 실측: '전체 기간' 화면 **제목 중복 0건**, 콘솔 에러 0, 군집 카드가 '동일 주제 N개'로 렌더.
  - 군집 표본 육안 확인 — 과병합 없음(국세청 호우 세정지원 18건, HD한국조선해양 LPG선 수주 13건 등
    모두 실제 동일 사건).
- **남은 한계(미조치):** 같은 사건이 여러 군집으로 갈리는 **미병합**이 있다. `_title_sig`의 토큰이
  `[0-9a-z가-힣]{2,}`라 `LPG 운반선`은 {lpg, 운반선}, `LPG운반선`은 {lpg운반선}으로 갈려 공통토큰이
  min_tokens(4)에 못 미친다. 실측 HD한국조선해양 LPG선 수주 25건이 3덩어리로 분리.
  라틴·한글 경계에서 부분토큰을 더하면 잡히지만 `_title_sig`는 **라이브 뉴스·산업이 함께 쓰는** 함수라
  회귀 위험이 있어 별도 루프로 미룬다.

---

## 2026-08-24 (29) — 워크플로가 남의 스트림 시각을 되돌리던 문제 + 감사인 표기 정규화

- **증상/계기:** ① 산업 수집 주기가 3시간이 아니라 **210분**으로 고정돼 있었다. ② `status.json`의
  타임스탬프가 되돌아갔다 — `03:32 run-all(news=03:31, ind=03:31)` 직후 `03:33 채용 갱신`이
  `news=03:01, ind=00:02`로 덮었다. ③ DART 감사인명이 같은 법인인데 `삼일회계법인 (PwC)`·
  `한영 회계법인`·`안진`처럼 갈려 19가지로 집계됐다.
- **무엇을 / 어디에:**
  - **게이트 반올림** — run-all이 30분 주기라 게이트 확인도 30분 단위로만 일어난다. 180분 게이트는
    직전 +179분 슬롯에서 1분 모자라 건너뛰고 다음 슬롯(+209분)에 걸린다.
    `industry_min_interval_minutes` **180 → 150**(`config.py`)으로 +179분 슬롯을 통과시켰다.
  - **시각 되돌림** — `scrape.yml`은 수동 전용이 아니라 **`src/**` 경로 push에도 트리거**된다.
    그 러너가 푸시 시점의 옛 `status.json`을 기준으로 자기 몫(jobs)만 갱신하고 나머지 스트림 시각은
    옛 값 그대로 써 넣는데, 커밋 단계의 `git pull --rebase -X theirs`가 그 옛 버전을 이기게 만든다.
    → 수집 워크플로 4개(`run-all`·`scrape`·`scrape-news`·`scrape-insights`) 모두 **export 직전에**
    `git pull --rebase --autostash origin <ref> || true` 를 넣어 남이 올린 status를 먼저 받게 했다.
  - **감사인 표기** — `adapters/dart.py`에 `_norm_auditor()` 신설(괄호 병기 제거 → `회계법인` 앞 공백 제거
    → 약칭 보정). `회계법인 리안`처럼 **상호 자체가 접두형**인 곳은 공백을 지우면 다른 이름이 되므로
    접미형일 때만 붙인다. 고정 테스트 `tests/test_dart_auditor.py`.
- **효과/검증:**
  - 되돌림 소멸 — 같은 "채용 갱신" 커밋 전후 실측: `03:33` news 03:31→**03:01 되돌림** /
    `04:13` news **04:01 유지**(jobs만 갱신).
  - 산업 주기 정상화 — `00:02 → 03:31 → 04:01`.
  - **DART CI 연동 확인** — `companies` 시각이 로컬 실행값(08-22T22:08)에서 **08-24T04:10**으로 전진.
    라이브 `companies.json` 실측 **188곳 / 감사의견 187 / KAM 178**, 삼일 감사대상 52곳.
  - 감사인 표기 **19종 → 16종**(3건 병합), `회계법인 리안` 보존. 전체 115 테스트 통과.
  - **부수 확인:** 외부 핑거(cron-job.org)는 22:01~03:31 30분 간격으로 한 번도 안 빠졌다. 정상이며,
    이번 수정은 워크플로 `steps:` 안쪽만 건드려 **외부 크론 설정 변경은 불필요**하다.
