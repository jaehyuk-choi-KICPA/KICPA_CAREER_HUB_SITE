...
- **알림 클릭 동선**: `sw.js notificationclick`이 **외부 공고는 새 창(`openWindow`)으로 연다** — PC=새 탭, 모바일/iOS=오버레이. 기존 회법몬 화면을 보존하고(모바일은 공고 닫으면 회법몬 복귀), iOS에서 메인뷰를 외부로 navigate하면 닫을 때 **빈 화면** 되던 문제도 방지. 내부 URL(홈 등)은 기존 탭 포커스·이동. *(과거 `?goto`로 히스토리 [홈,공고]를 만들어 '뒤로가기=홈'을 시도했으나 브라우저별 push/replace 동작 차이로 신뢰성이 없어 폐기 — 알림 1클릭당 창 1개 제약도 한계. 단순 '새 창'이 안정적.)*
