...
## 2026-08-22 (28) — 딜·M&A 카테고리 폐지 + 산업은 국내 기업만 + DART를 감사정보 중심으로

- **증상/계기:** ① 딜·M&A가 부동산·해외 소형딜 노이즈로 오염된 채 **아카이브 기사의 66%(7,287 중 4,822건)**를
  차지해 감사·세무를 시각적으로 묻어버렸다. ② 산업 탭에 YMTC IPO·러시아 원전·美 제조업 설비투자·앤트로픽
  IPO 같은 **한국 무관 기사**가 섞였다. ③ DART에 감사인 전용 API가 따로 있는 걸 뒤늦게 확인했다.
- **무엇을 / 어디에:**
  - **딜·M&A 삭제** — `news_queries`·`news_recent_days_by_category`·`news_foreign_filter_categories`에서 제거,
    `news_deal_exclude`와 export의 딜 전용 필터 블록 삭제, 프론트 `NEWS_CAT_ORDER`/`NEWS_CAT_COLOR` 정리.
    아카이브의 딜 기사 4,822건도 회수(**7,287 → 2,465**). git 이력에 남아 있어 되돌릴 수 있다.
  - **산업 = 국내 기업만** — `_foreign_blocked`(뉴스식 규칙)를 `_non_domestic`으로 교체.
    뉴스 쪽은 '미국·글로벌'을 keep 마커로 살리지만 산업엔 그 예외를 두지 않는다.
    판정: **외국 마커 있음 + 사전 기업 태그 없음 + 국내 마커 없음 → 제외.**
    덕분에 "삼성SDI 美 LFP 양산"·"LG엔솔 배터리 美 운송 수주"는 남고 "美 제조업 설비투자 5200조"는 빠진다.
    (태깅을 필터보다 **먼저** 수행하도록 순서를 바꿨다 — 국내 판정에 기업 태그가 필요하다.)
  - **DART 감사정보 중심 재편** — `flr_nm` 역추론을 버리고 전용 API로 교체:
    `accnutAdtorNmNdAdtOpinion.json`(감사인·감사의견·**핵심감사사항**·강조사항) +
    `adtServcCnclsSttus.json`(감사보수·감사시간). 3개년 재무표는 **뺐다**(화면이 무거운 데 비해
    DART 링크로 보는 편이 낫고, 감사 지원자에게 값진 건 KAM이다).
    DART 정식명이 통칭과 달라(NAVER·케이티·한국전력공사·POSCO홀딩스 등) config `dart` 필드로 8곳 보정.
- **⚠️ 사고 기록(재발 방지):** `news_deal_exclude` 블록을 정규식 `\[.*?\],\n`으로 지웠는데, 딜 목록이
  `],   # 주석`으로 끝나 lazy 매칭이 **다음 블록인 `news_require_any`까지 삼켰다.** 관련성 게이트가 꺼진 채
  1회 수집이 돌아 아카이브에 6건이 유입됐고, git 원본에서 블록을 복구한 뒤 그 6건을 회수했다.
  → **config 블록을 정규식으로 지우지 말 것.** 정확한 문자열 매칭을 쓰고, 지운 뒤 키 개수를 확인할 것.
- **효과/검증:** 뉴스 3분류 56건(딜 0), 산업 281건 중 외국 마커 잔존 8건은 **전부 한국 기업 기사**.
  DART 51곳 수집(핵심감사사항 47곳, 감사인 삼일 18·삼정 17·안진 8·한영 8). sitecheck 전 항목 OK.

---

## 2026-08-22 (27) — DART Open API로 기업 감사인·3개년 재무 수집

- **증상/계기:** 산업 화면에서 기업을 골라도 **DART 검색 링크로 보내는 것이 전부**였다.
  "관심산업군 → 기업 → 재무제표" 동선의 마지막 칸이 비어 있었다.
- **무엇을 / 어디에:** `src/adapters/dart.py` 신설 + `export --part companies` → `docs/data/companies.json`.
  스크래핑이 아니라 **공식 Open API**를 쓴다(안정적·합법적, 수치와 링크만 저장하므로 저작권 안전,
  결정론적 공공 API라 코어 LLM-free 원칙과 무관).
- **정찰 노하우(실측):**
  - 공시검색 `list.json` 응답의 **`flr_nm`(공시 제출인명)**이 감사보고서 건에선 곧 **그 회사의 감사인**이다.
    감사보고서는 감사인이 제출하기 때문. 규칙만으로 감사인을 알아내는 유일한 경로.
  - 유형을 나눠 조회해야 한다(A=정기공시 / F=외부감사관련). 한 번에 받으면 100건 상한에 밀려 감사보고서가 빠진다.
  - `fnlttSinglAcnt.json`은 `reprt_code=11011` **한 번 호출로 3개년**(당기/전기/전전기)을 준다. 2015년 이후.
  - `corpCode.xml`은 비상장 포함 10만 건대라 동명이인이 흔하다 → **stock_code 있는 상장사를 우선** 채택.
- **주기·비용:** 재무·감사인은 분기 단위로만 바뀌므로 주 1회 게이트(`companies_min_interval_minutes`).
  기업 60곳 ≈ 180콜, 한도는 일 20,000건이라 여유가 크다.
- **함정 2가지(둘 다 운영을 깨뜨림):**
  - 워크플로에서 `git add docs/data/companies.json`을 다른 경로와 같은 줄에 두면, 키가 없어 파일이
    생성되지 않았을 때 **스텝 전체가 죽는다** → 별도 줄에서 `|| true`로 실패를 흡수.
  - 저장소에 파일이 아예 없으면 프론트 fetch가 404를 내고 **sitecheck의 '콘솔 에러 없음'이 실패**해
    오탐 이슈가 열린다 → **빈 플레이스홀더**를 커밋해 둔다.
- **효과/검증:** 키 없는 상태에서 no-op 경로 확인(파일 미생성·프론트는 검색 링크만), 샘플 데이터로
  감사인 배지·3개년 표·공시 칩 시각 검증 후 샘플 삭제. sitecheck 전 항목 OK, 콘솔 에러 0.

---

## 2026-08-22 (26) — 누적 아카이브 계층 신설 + 2026-05부터 소급 구축

- **증상/계기:** `news.json`은 매 수집마다 **전량 덮어쓰기**라 보존창(21일)이 지나면 기사가 사라진다.
  사이트에 남는 건 "지금 이 순간의 스냅샷" 한 장(55건)뿐이고, 꾸준히 모아온 이력이 어디에도 없었다.
- **무엇을 / 어디에:**
  - `src/archive.py` 신설 — `docs/data/archive/{스트림}/{YYYY-MM}.json` **월별 샤드에 URL 기준 멱등 upsert**
    + `index.json`. `export.py`가 `_write_guarded` 직후 스트림별로 호출한다.
  - `scripts/backfill_archive.py` 신설 — ① git 스냅샷 복원(`git cat-file --batch`로 3,090장을 10~20초에)
    ② RSS `after:`/`before:` 기간 소급(리포 생성 이전 구간).
- **설계 판단 2가지:**
  - **월별+스트림별 샤딩**: 매 수집에 바뀌는 파일이 당월 1개뿐 → git 델타가 "말미 몇 줄 추가"로 수렴.
    통짜 파일이면 30분마다 수 MB blob이 통째로 재기록돼 리포가 급속히 부푼다.
  - **dupes 평면화**: 처음엔 군집을 그대로 담았더니 대표 레코드의 dupes가 스냅샷마다 합쳐지며
    **11MB(레코드당 2.2KB)**까지 부풀었다. 대표·멤버를 각각 독립 레코드로 펴자 **3MB**로 줄었고
    고유 건수는 그대로였다(멤버는 다른 스냅샷에선 대표라 이미 존재).
- **효과/검증:** 기사 **7,287건**(2026-05 541 / 06 2,038 / 07 2,689 / 08 2,019) 복원.
  RSS 소급이 5월을 83 → 541건으로 채운 것이 백필이 실제로 동작했다는 신호다.
  `tests/test_archive_merge.py` 16건(멱등·월버킷·하한·최초값 고정·1건1줄) 통과.

---

## 2026-08-22 (25) — 산업 스트림 신설 (회계·재무 렌즈 11개 산업 + 기업 태깅)

- **증상/계기:** 기사가 회계 도메인 4분류뿐이라 **산업·기업 축이 아예 없었다**.
  "관심산업군을 정해 특정 기업 재무제표를 분석"하는 동선을 사이트가 지원하지 못했다.
- **무엇을 / 어디에:** 기존 뉴스 파이프라인에 **섞지 않고** 별도 스트림으로 분리
  (`industry_*` config 네임스페이스 / `build_industry_adapters` / `export.build_industry` / `industry.json`).
  `build_news` 본문은 무수정 — 시그니처에 `results=None`만 추가(백필이 필터 체인만 재사용하는 훅).
- **왜 분리했나:** ① `news_require_any`가 산업 기사를 100% 드롭 ② `embeds._prototypes`가 `news_queries`
  값 전체를 프로토타입 임베딩해 기존 감사·세무 판정을 오염 ③ `_dedup_near`의 카테고리 무시 선점 잠식
  ④ `news_exclude`의 정치어가 에너지·건설 산업정책 기사를 오차단.
- **수집 튜닝(단계별 드롭 진단 후):**
  - 최대 손실원은 관련성 게이트가 아니라 **보존 컷오프**였다(자동차 120건 중 60건, 통신 79건 탈락)
    → `industry_recent_days` 14→30. **125 → 283건**.
  - 저수확 4산업(자동차·유통·금융·통신)에 **3번째 쿼리 풀** 추가(감사 2풀 분리와 같은 원리) → **360건**.
  - 단독 `"공모"`가 '사진 공모전'을 통과시켜 `공모주/공모가`로 교체, `[주간증시전망]`처럼 띄어쓰기 없는
    시황 변형도 차단. 산업 스트림 최대 노이즈원은 **증권가 시황·특징주**다(재무어를 다 포함해 게이트를 통과).
- **기업 태깅 오탐 방지 4규칙:** 최장일치+스팬 마스킹 / 라틴 별칭 영숫자 경계(`KTX` 차단) /
  짧은·strict 한글 별칭의 한글 경계(`기아대책본부` 차단) / 법인 접미어(`셀트리온제약`·`현대차그룹` 차단).
  `tests/test_industry_tag.py`에 고정 케이스로 박아 뒀다.
- **효과/검증:** 11개 산업 12~38건 균형, 기업 26곳 태깅, 소스 26/26 정상. 시각 검증 완료.

---

## 2026-08-18 (24) — 기사: 정치 필터 2차 보강 (한자 약칭·전당대회·지방의회)

- **증상/계기:** 감사 카테고리에 정치기사 2건 침투 — 정치인이 **'분식회계'를 비유로 쓴 정당 공방**("송영길, 정청래 직격 [현장영상]", "與 8·17 전당대회…'지선 승리 주장은 분식회계'")이 감사 쿼리(분식회계)에 걸림. 7/21 정치 필터의 한글 토큰(여야·여당·민주당 등)엔 **한자 약칭(與)·전당대회·인명**이 없어 통과. 딜엔 지자체 행정기사(고창 복분자유원지 매각…**고창군의회** 보류)도 잔존.
- **무엇을 / 어디에:** `news_exclude` += **전당대회·합동연설회·[현장영상]**(정당 행사·방송 정치클립), **"與 "·"野 "(공백 동반)·與野**(한자 정당 약칭 — 한글 '참여' 무관), **시의회·군의회·구의회·도의회**(지방의회 행정). 인명(송영길 등)은 브리틀해서 미등록 — 행사·클립 마커로 커버. + `news_exclude_sources` += **Hypebeast**(패션 매체의 LA Lakers 구단 지분 매각 등 무관 딜).
- **효과/검증:** 라이브 53건 전량 시뮬레이션 — 노이즈 4건(정치 2·지방의회 1·Lakers 1)만 제거, **정책 실질 기사 오차단 0**(영풍 증선위 제재·지정감사 개편·사모펀드 규제 법안·종부세 정책 모두 보존). 다음 run-all부터 반영. [[insight-news-relevance-gate]]

---

## 2026-07-21 (23) — 기사: 딜·M&A 쿼리 오염 보강 (부동산·리포트·SPAC·中 IPO)

- **증상/계기:** 라이브 딜·M&A 20건 중 6건이 노이즈 — ① 오피스 부동산 매각(GS건설 서초타워), ② 지자체 공공자산 매각 정치기사(북구), ③ 증권사 목표가 리포트(YG·스튜디오드래곤 — '밸류에이션' 쿼리 오매칭), ④ 미국 SPAC 쉘 공시 번역물(DT CLOUD STAR ACQUISITION CORP), ⑤ 중국 IPO('[中 하드테크…]' — 외국 필터의 "中," 쉼표 조건 미매칭).
- **무엇을 / 어디에:** `config.dashboard` 규칙만 보강(쿼리 자체는 유지 — 쿼리 축소는 정상기사 유실 위험).
  - `news_deal_exclude` += 사옥/빌딩/타워 매각·용적률(부동산), 공공자산(지자체), 목표가·목표주가·투자의견(애널리스트 리포트), ACQUISITION CORP(SPAC — 제목 매칭이 case-sensitive라 대소문자 2표기).
  - `news_foreign_countries` += **"中 "(공백 동반)** — 기존 "中,"(쉼표)만으론 '[中 하드테크 IPO]'류 미차단. 붙여쓰는 中企·中소는 여전히 안전.
- **효과/검증:** 라이브 73건 전량 시뮬레이션 — **노이즈 6건 전부 제거, 정상 딜 14건 오차단 0**(가비아 EBITDA 밸류·야놀자 나스닥·우버-딜리버리히어로 등 유지). 타 카테고리(세무·감사·채용) 매칭 0건. 다음 run-all부터 라이브 반영. [[insight-news-relevance-gate]] [[insight-news-foreign-filter]]
- **(후속, 같은 날) 정치 이슈 연루 기사 제외:** `news_exclude`에 정치 토큰 추가 — 정무위·국정감사·국감·청문회·인사청문·특검·탄핵·대선·총선·지방선거·여야·야당·여당·민주당·국민의힘·공약·개각·**" 의원"(공백 동반)**·대통령. ⚠️ 캘리브레이션 실측: **'국회'·'의원' 단독은 금지** — "미지정 회계사, 국회도 문 연다"(핵심 채용기사)·"병의원 부가가치세"(정상 세무기사)가 오차단됨. 공백+의원은 인명 호칭('○○○ 의원')만 잡고 병의원은 통과. 라이브 73건 시뮬레이션: 정치 프레임 2건(홈플러스 정무위 공방·'대통령처럼' 클릭베이트)만 제거, 오차단 0.
...
- **무엇을 / 어디에:**
  ...
  - (후속2, 사용자 결정) 자격무관 강등으로도 부족 — **AICPA 타깃 공고는 목록에서 아예 제외**. `filters.title_exclude_keywords`에 **aicpa·미국회계사·미국공인회계사** 추가(제목 기준, 면제 소스보다 우선). 병기 제목("KICPA/AICPA …")도 드롭되는 트레이드오프 수용. 라이브 74건 회귀: 서현 건 1건만 드롭.
...
- **무엇을 / 어디에:**
  - `news_exclude_sources`에 **지역 일간지 대거 추가**(부분일치) — 전국지·전문지(세정일보·한국세정신문·법률신문·조세금융신문 등)는 유지.
  - `news_exclude`에 **실무교육·보수교육·세무설명회·[프로필]** 추가(지역 교육·인사 노이즈).
  - 채용 쿼리에 **'미지정'** 추가 — "○○, 미지정 합격자 채용"처럼 *제목에 '회계사'가 없는* 채용 기사 포착 목적. `news_require_any` 게이트가 비(非)회계 '미지정'(주차·좌석 등) 노이즈를 차단.
- **효과/검증:** 세무 지역지/교육/프로필 제거(정상 전문지·전국지는 보존). 채용·시험 8→**20**(다만 같은 피드 A/B에서 바 '미지정'은 **중립**: 기존 '미지정 회계사'가 이미 커버 — 8→20은 피드 surge가 주원인. 바 '미지정'은 위 엣지케이스 대비로 유지, require_any로 가드). [[insight-news-volume-tuning]] [[insight-news-relevance-gate]]

---

## 2026-06-28 (17) — 기사: 지역지(도민일보) 출처 + 인사 동정(승진) 제외

- **증상/계기:** 지역지 **강원도민일보**발 "국세청 ○○과장 부이사관 **승진**"처럼 *시시콜콜한 인사 동정* 기사가 세무에 섞임(상속증여세과장 키워드로 매칭).
- **무엇을 / 어디에:** `news_exclude_sources`에 **"도민일보"**(경남·강원·충청 등 지역지 일괄), `news_exclude`에 **"승진"·"영전"**(인사 동정) 추가.
- **효과/검증:** 재수집 시 도민일보 출처·승진/영전 제목 **0건**. 기존 동정·위촉·임명식 노이즈군과 동일 성격. [[insight-news-relevance-gate]]

---

## 2026-06-28 (16) — 기사: 딜·M&A '매각'이 끌어온 부동산성 노이즈 제외(카테고리 스코프)

- **증상/계기:** 딜·M&A 쿼리의 일반어 **'매각'**이 *부동산·압류재산성 매각*까지 끌어옴 — 예: "[캠코 공매] …'아파트' 등 **압류재산 매각**", "한성숙 **오피스텔** 지인에 헐값 매각"(정치성). 타깃(딜 업계)과 무관.
- **무엇을 / 어디에:** 쿼리에서 '매각'을 빼면 정상 딜(계열사·지분·경영권 매각)을 잃으므로 **유지**하고, **딜 카테고리 한정 제외** `news_deal_exclude`(아파트/부동산/주택/토지/상가 매각·오피스텔·빌라 매각·압류재산·분양권 + 정치성 **대가성**) 추가. `export._build_news`에서 `n.category=="딜·M&A"`일 때만 적용 → **세무의 '부동산 매각 양도세' 등 정상기사는 다른 카테고리라 무사**(스코프 안전). '헐값 매각'은 정상 딜에도 있어 제외 불가 → 부동산어/정치마커로 우회.
- **효과/검증:** 재수집 시 딜 부동산·정치 노이즈 **0**("압류재산 아파트 공매", "한성숙 오피스텔·집 헐값 매각 대가성" 제거), 정상 딜("SK스퀘어 원스토어 626억 헐값 매각"·M&A·IPO)·"한화 판교 연구소 매각"류는 보존. [[insight-news-relevance-gate]] [[insight-news-four-categories]]

---

## 2026-06-26 (15) — 기사: '한공회'를 쿼리 대신 재분류 목록으로(자기잠식 회피)

- **증상/계기:** 채용·시험 쿼리에 약칭 **'한공회'**가 없어 추가 요청. 그런데 쿼리에 직접 넣으니 같은 피드 A/B에서 **채용·시험 19→12**(한공회 제목 2건 얻고 7건 잃음) — [[insight-news-volume-tuning]]의 OR 자기잠식 재현.
- **무엇을 / 어디에:** 쿼리 추가를 **기각(원복)**하고, `config.dashboard.news_hire_title_keywords`에 **"한공회"**만 추가. 다른 쿼리로 들어온 한공회 기사를 **채용·시험으로 재분류**(RSS 결과 불변 → 자기잠식 0).
- **효과/검증:** 재수집 시 채용·시험 **19 유지**, 한공회 기사("회계사 합격자, 한공회 신청시 수습처 배정")가 채용·시험으로 정확 분류. 교훈: **고신호 동의어/약칭은 쿼리 확장 말고 재분류 목록으로**. [[insight-news-category-correction]] [[insight-news-relevance-gate]]
...
- **무엇을 / 어디에:**
  - **법인 PR 필터(AND 규칙)** — `config.dashboard.news_firm_pr_entities`(세무법인·회계법인·세무사사무소…) + `news_firm_pr_actions`(오픈·개소·개업·현판·분사무소·**출간·발간**). `export._build_news` 필터 루프(지자체 gov_action 블록 다음)에 **둘 다 있으면 drop**. 개업/지점 개소 홍보 + **법인 대표 책·실무서 출간 홍보**(예: "○○회계법인 대표 … 실무서 2권 동시 출간")까지 포함. 단독어(`개업`·`오픈`·`출간`) 차단은 '사업자 개업신고'·'오픈마켓 부가세'·'백서 출간' 등 정상기사 오차단 위험 → **법인어 동반 시에만** 컷.
