...
1. **채용공고** — KICPA 2보드 + 삼정·안진·한영·삼일(6어댑터). 분류: 법인(삼일/삼정/안진/한영/로컬/기타) ×
   **자격요건(수습CPA/자격무관)** × **채용구분(인턴/정규직/계약직/파트타임)** × 상태(진행중/마감) × NEW(게시 24h).
   ...
   방금 올라온 공고 패널은 책갈피로 **2026 신규공채** 탭과 전환(내부 id는 `big4` 유지 — 딥링크 `?view=big4` 하위호환) —
   ① **빅4 신입 회계사 공채**(삼일·삼정·안진·한영 정기채용 일정, 접이식·기본 닫힘): **수동 큐레이션 `docs/data/big4_recruit.json`**
   (export 파이프라인과 분리, 시즌마다 수기 갱신 — 섹션 제목은 정적 라벨 고정, JSON title 미주입). 법인색 좌측 바.
   ② **로컬 신입 회계사 공채**(접이식·기본 열림): jobs.json에서 로컬×수습CPA×미마감(제목 '(마감)' 가드 + 제목 '회계사/CPA' 게이트 — 수습 보드의 총무·기장 공고 오염 차단)을 자동 필터해
   **풀타임 ↔ 파트타임 하위 탭**(각 50% 폭)으로 표시(마감 임박순, 5개+더보기 — `renderLocalRecruit`).
   빅4 섹션 summary엔 접수 중 법인 있으면 '접수중' 딱지, NEW가 비면 이 탭이 기본 화면(부트 자동 전환).
   PC 와이드에선 우측 패널(.today-col)이 sticky+자체 스크롤(max-height 100vh 기준 — 본문과 독립).
...
2. **기사(업계)** — Google News RSS(`adapters/news_rss.py`), **3분류(채용·시험/감사/세무)**. 제목+출처+링크만, 노이즈 제외, 기본 21일 보존(카테고리별 차등).
   (딜·M&A는 2026-08-22 폐지 — 부동산·해외 소형딜 노이즈가 심하고 아카이브의 66%를 차지해 다른 분류를 묻었다.)
2-1. **산업 기사** — **뉴스와 분리된 별도 스트림**(`industry_*` config → `build_industry` → `docs/data/industry.json`).
   회계·재무 렌즈(실적·수주·증설·증자·M&A·손상·구조조정)로 산업 11개를 모으고, 기업 사전(57곳)으로 **기업 태그**를 붙여
   **국내 기업 기사만** 남긴다(외국 마커 + 기업태그 없음 + 국내마커 없음 → 제외. 뉴스와 달리 '미국·글로벌' 예외 없음).
   `산업군 → 기업 → DART 전자공시` 동선을 만든다. ⚠️ **절대 `news_queries`에 산업을 넣지 말 것** —
   `news_require_any`가 전량 드롭하고, `embeds._prototypes`가 기존 감사·세무 판정을 오염시키며,
   `_dedup_near`가 카테고리를 무시해 선점 잠식이 난다. 어댑터가 26개라 `industry_min_interval_minutes`(180)로 3시간에 1회만 수집.
3. **빅펌 인사이트** — 삼일·삼정·안진·한영 간행물(`insights.py`). 사이트가 JS(SPA)라 **Playwright 헤드리스**(`render.py`)로 렌더 후 링크 추출.
   (프론트는 **한 탭('기사·리포트') 안 책갈피 3개** — 📰업계 / 🏭산업 / 📑리포트. 백엔드 데이터는 그대로 분리.)
3-1. **기업 정보(DART)** — `src/adapters/dart.py`가 **공식 Open API**로 기업별 **감사인·감사의견·핵심감사사항(KAM)·
   강조사항·감사보수·감사시간**을 모아 `docs/data/companies.json` 생성(주 1회). 재무 수치는 일부러 담지 않는다
   (화면이 무거워지는 데 비해 DART 링크로 보는 편이 낫고, 감사 지원자에게 값진 건 KAM이다).
   스크래핑 아님 = 안정적·합법적, 수치와 링크만 저장(저작권 안전). **`DART_API_KEY` 없으면 기능 전체 비활성**
   (파일 미생성, 프론트는 DART 검색 링크만 → 사이트 정상). ⚠️ 저장소엔 **빈 플레이스홀더**를 둔다 —
   없으면 프론트 fetch 404가 sitecheck '콘솔 에러 없음'을 실패시켜 오탐 이슈가 열린다.
4. **누적 아카이브** (`src/archive.py`) — 위 스트림들은 매 수집마다 전량 덮어쓰기라 보존창이 지나면 기사가 사라진다.
   그 손실을 받아내는 계층으로 `docs/data/archive/{스트림}/{YYYY-MM}.json` **월별 샤드에 멱등 append**(+ `index.json`).
   프론트는 각 화면 상단 **기간 필터(최근 ↔ 전체 기간)**로 열람하며, 아카이브는 '전체 기간'을 누를 때만 lazy load.
   하한 `archive_since`(2026-05-01). 소급 구축은 `scripts/backfill_archive.py`(git 스냅샷 복원 + RSS `after:`/`before:`).
   ⚠️ 군집(dupes)은 **평면화해 저장**하되 대표 URL을 `gid`로 남긴다 — 중첩하면 스냅샷마다 합쳐지며
   무한 누적되고(실측 11MB → 3MB), 관계까지 버리면 화면이 중복 범벅이 된다(프론트 `foldByGid`가 다시 접음).
   같은 기사가 다른 구글 링크로 재유입되는 것은 `merge_items`의 **제목 가드**가 막는다(과거분은 `scripts/archive_dedup.py`).
- **개인정보처리방침(`docs/privacy.html`)** — 수집 파이프라인과 무관한 **별도 정적 페이지**. 푸터 저작권 줄에
  작은 링크로만 걸고 본문 탭에는 넣지 않는다. 사이트가 실제로 다루는 방문자 데이터는 **GA4 쿠키**와
  **웹푸시 구독(endpoint·keys·scope)** 둘뿐. 애널리틱스·푸시 저장항목·외부 서비스·localStorage 키를
  바꾸면 이 페이지와 하단 시행일도 함께 고친다(GA 약관이 고지·게시를 요구).
...
- **이용 설문 안내 팝업(프론트 전용·v1.21)** — 접속 2초 뒤 1회(PC 카드/모바일 시트). 캠페인은 `app.js`의 `SURVEY` 상수(`id`·`until` 지나면 자동 중단),
  닫기=탭 세션·'1주일간 보지 않기'=7일·참여=30일(`survey_closed`/`survey_hide`). **`navigator.webdriver`면 미표시**(sitecheck 탭 클릭 보호), `?survey=1`로 강제 미리보기. 운영=사용설명서 §설문 팝업.
...
## 구조 · 워크플로우
> 파일 맵·전체 파이프라인·모니터링(통합 `monitor.yml` 1일 1회 + freshness 1h)·채용알림(웹푸시)·외부 핑거 설정은 **`docs-meta/WORKFLOW.md`** 참조.

⚠️ **GitHub 무료 public cron은 자주 드롭됨** — 정기 수집 주경로는 **외부 핑거(cron-job.org)**가 `run-all.yml`을 호출(수집 후 **채용알림 푸시 발송**도 수행).  
수집 개별 yml(`scrape·scrape-news·scrape-insights`)은 수동 전용. 모니터링은 **통합 `monitor.yml`(1일 1회, KST 07:00)**이 주축. freshness 1h(LLM 없음=무료)는 실행 감시로 병행, sitecheck cron은 폐기(수동 전용). LLM 비전은 최저가 haiku 사용(토큰 절감, 2026-07).
...
### 자동화 신뢰성 = 통합 monitor.yml(1일 1회) + freshness 1h (변경 시 함께 고려)
1. **실행됐나** → `freshness.py` 2. **수집됐나** → `canary.py` (수동, 의도-인지 필수) 3. **제대로 보이나** → `sitecheck.py` → **`monitor.yml`(1일 1회)이 canary+sitecheck 통합 점검**
